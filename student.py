...
# 1. إعدادات المظهر وإخفاء أدوات Streamlit
...
if st.session_state.user_role is None:
    ...
    if st.button("دخول"):
        if role == "المالك" and username == "admin" and password == "123":
            st.session_state.user_role = "owner"; st.session_state.user_name = "الإدارة العامة"; st.rerun()
        else:
...
# اللوحات بعد الدخول
    ...
    if st.sidebar.button("تسجيل الخروج"): st.session_state.user_role = None; st.rerun()
    ...
    if st.session_state.user_role == "owner":
        ...
        with tab1:
            ...
            if st.button("حفظ المعلمة"):
                if insert_data("Teachers", ["T"+str(datetime.now().microsecond), name, passwd]): st.success("تم!")
        with tab2:
            st.dataframe(fetch_data("Teachers"))
    ...
    # 2. لوحة المعلمة
        ...
        tab1, tab2, tab3 = st.tabs(["📝 رصد الدرجات", "➕ تسجيل طالبة", "📊 تقارير"])
        with tab1:
            if not my_students.empty:
                ...
                grade_h = st.text_input("درجة الحفظ:")
                grade_m = st.text_input("درجة المراجعة:")
                rep = st.text_area("تقرير الحفظ:")
                new_h = st.text_area("الحفظ الجديد:")
                if st.button("حفظ الرصد"):
                    if insert_data("Attendance_Grades", [s_id, datetime.now().strftime("%Y-%m-%d"), grade_h, grade_m, rep, new_h]): st.success("تم الحفظ!")
            else: st.info("لا توجد طالبات مسجلات في صفك.")
        with tab2:
            s_name = st.text_input("اسم الطالبة:")
            s_pass = st.text_input("كلمة المرور:", type="password")
            s_halaqa = st.text_input("اسم الحلقة:")
            if st.button("تسجيل"):
                if insert_data("Students", ["STU-"+str(datetime.now().microsecond), s_name, s_pass, s_halaqa, str(st.session_state.user_id)]): st.success("تمت الإضافة!")
        with tab3:
    ...
    # 3. لوحة الطالبة
        ...
        st.title(f"🌸 أهلاً {st.session_state.user_name}")
        ...
        if not df_grades.empty:
            ...
            if not my_grades.empty:
                ...
                pdf_data = generate_pdf("Report", [f"Date: {row['Date']} | Hifz: {row['Hifz_Grade']}" for _, row in my_grades.iterrows()])
